...
```
Jogo de batalha por turnos desenvolvido em Python com Tkinter, criado como
projeto final da disciplina de Programação II.
```

```
Inspirado em jogos como Pokémon e Monster Hunter, o jogador escolhe um micro-
organismo e o leva para batalhar contra inimigos controlados pela IA.
```
...
`🧬 Vírus  Bactéria  Fungo` 🟢 `Bactéria  Fungo  Vírus` 🟠 `Fungo  Vírus  Bactéria` 
...
`###` 🟢 `Bactérias — Equilibradas e resistentes Nome  HP  Defesa  Velocidade -----------------------------Lactobacillus  125  12  20 KPC (Superbactéria)  120  18  15 Salmonella  105  14  22` 

```
 🧬 Habilidade Passiva — Fissão Recupera 5 PE sempre que causar dano.
```

`###` 🟠 `Fungos — Lentos, mas extremamente resistentes Nome  HP  Defesa  Velocidade -----------------------------Penicillium  120  14  14 Candida auris  115  16  12 Cogumelo Proibido  110  18  10` 

```
 🧬 Habilidade Passiva — Quitina Reduz todo dano recebido em +3 pontos extras
além da defesa normal.
```
...
```
  - Polimorfismo — cada subclasse sobrescreve métodos como `bonus_ataque()`,
`reducao_defesa()` e `habilidade_passiva_descricao()`
```
